...
)
{
    ...
    //stbtt_fontinfo fi;
    if (!stbtt_InitFont(&fontOut->finfo, ttf_buf, stbtt_GetFontOffsetForIndex(ttf_buf, 0)))
    {
        return false;
    }

    int ascent, descent, lineGap;
    stbtt_GetFontVMetrics(&fontOut->finfo, &ascent, &descent, &lineGap);

    // Convert the font's "font units" to pixels proportional to bakePxH size:
    float scaleForBake = stbtt_ScaleForPixelHeight(&fontOut->finfo, bakePxH);
    ...
    void *userData)
{
    ...
    if (!fontData->cdata)
    {
        ...
        return CLAY__INIT(Clay_Dimensions) {.width = 0, .height = 0};
    }
    ...
    return CLAY__INIT(Clay_Dimensions) {
        .width = x,
        .height = y + lineH,
    };
}
    ...
    void *userData)
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        v[0] = {x0, y0, u0, v0, cr, cg, cb, ca, fontToUse};
        v[1] = {x1, y0, u1, v0, cr, cg, cb, ca, fontToUse};
        v[2] = {x0, y1, u0, v1, cr, cg, cb, ca, fontToUse};

        v[3] = {x0, y1, u0, v1, cr, cg, cb, ca, fontToUse};
        v[4] = {x1, y0, u1, v0, cr, cg, cb, ca, fontToUse};
        v[5] = {x1, y1, u1, v1, cr, cg, cb, ca, fontToUse};
