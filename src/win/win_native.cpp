...
WinNative::WinNative()
    ...
    reg_window_ = CreateWindow(szWindowClass, L"win native reg window", 0, CW_USEDEFAULT,
                               CW_USEDEFAULT, 0, 0, nullptr, nullptr, 0, nullptr);
}
...
void WinNative::RegisterHotKey(uint32_t key, uint32_t modifier) {
    ...
    if (registered_keys_.find(key_combo) == registered_keys_.cend()) {
        ...
        if (!key || !modifier) {
            printf("Failed to register hotkey, couldn't map the given GLFW key with the native "
                   "platform virtual key...\n");
            return;
        }
...
uint32_t WinNative::GetModKeyFrom(uint32_t g_key) {
    static std::unordered_map<uint32_t, uint32_t> m_key_map = {
        {MapVirtualKey(VK_LCONTROL, 0), MOD_CONTROL}, {MapVirtualKey(VK_RCONTROL, 0), MOD_CONTROL},
        {MapVirtualKey(VK_LMENU, 0), MOD_ALT},        {MapVirtualKey(VK_RMENU, 0), MOD_ALT},
        {MapVirtualKey(VK_LSHIFT, 0), MOD_SHIFT},     {MapVirtualKey(VK_RSHIFT, 0), MOD_SHIFT}};
    if (m_key_map[g_key])
...
uint32_t WinNative::GetKeyFrom(uint32_t key, uint32_t type)
{
    ...
    {
        // number keys..
        for (auto num_pad_key = VK_NUMPAD0, glfw_num = GLFW_KEY_KP_0;     num_pad_key <= VK_NUMPAD9;
num_pad_key++, glfw_num++) glfw_key_map[MapVirtualKey(num_pad_key, 0)]                = glfw_num;
        for (WCHAR num_key = '0',           glfw_num = GLFW_KEY_0;        num_key <= '9'; num_key++,
glfw_num++) glfw_key_map[MapVirtualKey(LOBYTE(VkKeyScan(num_key)), 0)] = glfw_num;
        // f keys..
        for (auto f_key = VK_F1,            glfw_f = GLFW_KEY_F1;         f_key <= VK_F24; f_key++,
glfw_f++) glfw_key_map[MapVirtualKey(f_key, 0)]                      = glfw_f;
        // chars
        for (WCHAR a_l = 'a',               glfw_al = GLFW_KEY_A;         a_l <= 'z'; a_l++,
glfw_al++) glfw_key_map[MapVirtualKey(LOBYTE(VkKeyScan(a_l)), 0)]     = glfw_al; for (WCHAR a_c =
'A',               glfw_ac = GLFW_KEY_A;         a_c <= 'Z';                a_c++, glfw_ac++)
...
bool WinNative::OnKeyPress(KBDLLHOOKSTRUCT* info)
{
    if (last_key_.is_down)
    {
        ...
        auto checker = [](uint32_t modifier, uint32_t key, const std::unordered_map<std::string,
RegKey>& map)
        {
            std::string key_combo = std::to_string(modifier) + "+" + std::to_string(key);
    ...
    CHECK:
        ...
        EventBus::Instance().publish(new HotkeyEvent(glfw_key, glfw_modifier,
Application::GetTotalRunningTime())); return true;
    }
    else if (last_key_.key == 0)
    {
        for (auto& r_it : registered_keys_)
        {
            ...
            if (reg_key.modifier == 0 && (reg_key.key == info->vkCode || reg_key.key ==
info->scanCode))
            {
                EventBus::Instance().publish(new HotkeyEvent(GetGLFWKeyFrom(info->vkCode), 0,
Application::GetTotalRunningTime())); return true;
            }
