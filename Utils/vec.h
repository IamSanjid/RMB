...
struct v2d_generic {
    ...
    // v2d_generic  max(const v2d_generic& v) const { return v2d_generic(std::max(x, v.x),
    // std::max(y, v.y)); } v2d_generic  min(const v2d_generic& v) const { return
    // v2d_generic(std::min(x, v.x), std::min(y, v.y)); }
    v2d_generic cart() {
